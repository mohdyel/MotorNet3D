...
class DVPP3D(nn.Module):
    ...
    def __init__(self, in_channels, embed_dim):
        ...
        # LayerNorm + final classifier head will be defined in the calling class
...
class TransSegClassifier(nn.Module):
    def __init__(self, pretrained_3d_ckpt, num_classes, dropout_rate=0.15):
        ...
        # Step 5: Replace segmentation head → classification head (unchanged but now receives DVPP output)
    ...
    def forward(self, x):
        ...
        # Originally, code would have flattened feat → tokens and passed through Transformer blocks:
        # cls_token = self.backbone.cls_token.expand(B, -1, -1)  # (B,1,embed_dim)
        # x_tok = feat.flatten(2).transpose(1,2)              # (B, num_patches, embed_dim)
        # x_tok = torch.cat((cls_token, x_tok), dim=1)
        # x_tok = x_tok + self.backbone.pos_embed
        # for blk in self.backbone.blocks:
        #     x_tok = blk(x_tok)
        # x_tok = self.backbone.norm(x_tok)
        # cls_out = x_tok[:, 0]                                  # (B, embed_dim)
        # logits = self.cls_head(cls_out)                        # (B, num_classes)
        # return logits

        # DVPP path (#DVPP: Step 2.2)  
        # Bypass the transformer CLStoken path entirely—feed DVPP vector into cls_head:
        logits = self.cls_head(dvpp_feat)                      # (B, num_classes)
        return logits
...
class MedViT3D(nn.Module):
    def __init__(self, pretrained_2d_ckpt, num_classes, image_size=(1,64,256,256),
        ...
        # Step 19: Replace classification head (takes DVPP output instead of 'cls_token')
    ...
    def forward(self, x):
        ...
        # Originally, code would flatten feat → tokens → transformer blocks → cls_token
        # x_flat = feat.flatten(2).transpose(1,2)    # (B, num_patches, embed_dim)
        # cls_token = self.cls_token.expand(B, -1, -1)
        # x_tok = torch.cat((cls_token, x_flat), dim=1)
        ...
        logits = self.head(dvpp_feat)                # (B, num_classes)
        return logits
